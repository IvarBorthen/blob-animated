...
class Blob {
    constructor({ canvas, color, colorFunction, vectors, speed = 200, scramble = 0.1, autoPlay = true, size = 1000, maskedElement, debug = false, inverted = false, changedVectorsCallback, }) {
        ...
        this._drawImage = () => {
            if (this._maskedElement && this._ctx) {
                ...
                if (width > height) {
                    ...
                    try {
                        this._ctx.drawImage(this._maskedElement, x < 0 ? 0 : x * sizeMultipler, y < 0 ? 0 : y * sizeMultipler, sizeMultipler * (width - x * 2), sizeMultipler * (height - y * 2));
                    }
                    catch (err) {
        ...
        this._draw = () => {
            if (this._ctx) {
                const { _ctx, _points, _numberOfPoints, _getCurvedPathPoints, _updatePositions, _inverted, } = this;
